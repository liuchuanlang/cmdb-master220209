...
class Tester(models.Model):
    project_name = models.CharField(max_length=64, unique=True, verbose_name='软件/系统版本')
    sub_tester_type_chaoice = (
        (0, 'PC'),
        (1, 'wap'),
        (2, 'applet'),
    )

    sub_tester_type = models.SmallIntegerField(choices=sub_tester_type_chaoice, default=0, verbose_name="平台")
    create_role = models.CharField(max_length=128, verbose_name="创建人")
    c_time = models.DateTimeField(auto_now_add=True, verbose_name='批准日期')
    m_time = models.DateTimeField(auto_now=True, verbose_name='更新日期')
    def __str__(self):
        return '%s--%s' % (self.get_sub_tester_type_display(), self.project_name)


class TestInterface(models.Model):

    interfaceid = models.CharField(max_length=64, primary_key=True, unique=True, verbose_name='接口编号')
    interfaceurl = models.CharField(max_length=200, unique=True, verbose_name='接口地址')
    interfacetype = models.CharField(max_length=64, verbose_name='请求类型')
    paramtype_choice = (
        (0, "Json"),
        (1, "Text"),
    )
    paramtype = models.SmallIntegerField(choices=paramtype_choice, default=0, verbose_name="参数类型")
    testdata = models.CharField(max_length=3000, verbose_name='测试数据')
    Expectedresults = models.CharField(max_length=64, verbose_name='预期结果')
    testresults = models.CharField(max_length=64, verbose_name='测试结果')

    def __str__(self):
        return '%s--%s--%s--%s--%s--%s' % (self.interfaceid, self.interfaceurl, self.interfacetype, self.get_paramtype_display(), self.testdata, self.Expectedresults)
